...
(() => {
  ...
  const ensureSharedTilt = (() => {
    let shared = null;

    const clampTilt = (value, limit) => clamp(value / limit, -1, 1);
    ...
    const bindOrientation = (store) => {
      ...
      const onOrientation = (event) => {
        ...
        const rawX = clampTilt(event.gamma, 28);
        const rawY = clampTilt(event.beta, 28);
        const rotated = rotateByScreen(rawX, rawY);

        store.targetX = rotated.x;
        store.targetY = rotated.y;
        store.supported = true;
        store.active = true;
        store.lastUpdate = performance.now();
      };

      store._orientationHandler = onOrientation;
      window.addEventListener("deviceorientation", onOrientation, { passive: true });
    };

    const armGesturePermission = (store) => {
      if (store._gestureArmed) return;
      store._gestureArmed = true;

      const requestFromGesture = () => {
        store.requestPermission();
      };

      window.addEventListener("touchstart", requestFromGesture, { passive: true, once: true });
      window.addEventListener("pointerdown", requestFromGesture, { passive: true, once: true });
      window.addEventListener("click", requestFromGesture, { passive: true, once: true });
    };

    return () => {
      if (shared && shared.__ready) return shared;
      if (window.__portfolioTilt && window.__portfolioTilt.__ready) {
        shared = window.__portfolioTilt;
        return shared;
      }
      ...
      shared = window.__portfolioTilt || {
        ...
        read() {
          ...
          const fresh = (performance.now() - this.lastUpdate) < 1600;
          if (!fresh) {
            this.targetX *= 0.92;
            this.targetY *= 0.92;
            this.active = false;
        ...
        requestPermission() {
          ...
          try {
            if (typeof DeviceOrientationEvent !== "undefined"
              ...
              this._requestInFlight = DeviceOrientationEvent.requestPermission()
                .then((state) => {
            ...
            const ok = start();
            this._requestInFlight = Promise.resolve(ok).finally(() => {
              this._requestInFlight = null;
            });
            return this._requestInFlight;
          } catch {
            this.permissionState = "denied";
            this._requestInFlight = Promise.resolve(false).finally(() => {
              this._requestInFlight = null;
            });
            return this._requestInFlight;
          }
  ...
  const sharedTilt = ensureSharedTilt();
  ...
  const updateTeachingLayers = (timeMs) => {
    ...
    const tiltState = sharedTilt.read();
    ...
    teaching.items.forEach((item, index) => {
      ...
      const tiltPushX = tiltState.active ? (tiltState.x * item.mouseX * 1.15) : 0;
      const tiltPushY = tiltState.active ? (tiltState.y * item.mouseY * 1.15) : 0;
